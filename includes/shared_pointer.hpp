...
template <typename T>
SharedPointer<T>::SharedPointer(T* address, kSharedPointerType type): holder_(address), type_(type){
...
SharedPointer<T>& SharedPointer<T>::operator=(const SharedPointer<T>& source) {
  if (&source == this || (source.holder_ == nullptr && source.ref_count_ != nullptr)) {
    return *this;
  }
  if (holder_ != nullptr) {
    *ref_count_ = *ref_count_ - 1;
